...
#Modified this file for both Python 2.7 and Python 3 compatibility
...
class XcodeProject(PBXDict):
    ...
    def remove_file(self, id, recursive=True):
        ...
        if id in self.objects:
            ...
            buildFiles = [f for f in list(self.objects.values()) if f.get('isa') == 'PBXBuildFile']
            for buildFile in buildFiles:
                if id == buildFile.get('fileRef'):
                    key = buildFile.id
                    PBXRBP = [f for f in list(self.objects.values()) if f.get('isa') == 'PBXResourcesBuildPhase']
                    PBXSBP = [f for f in list(self.objects.values()) if f.get('isa') == 'PBXSourcesBuildPhase']
...
if sys.version_info[0] > 2:
	class PBXWriter(plistlib._PlistWriter):
		def writeValue(self, value):
			if isinstance(value, (PBXList, PBXDict)):
				plistlib._PlistWriter.write_value(self, value.data)
			else:
				plistlib._PlistWriter.write_value(self, value)
